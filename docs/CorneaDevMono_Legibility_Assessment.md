...
This document records the design intent behind CorneaDev Mono and the published evidence that supports it. It has two jobs: to guide future design and engineering work against a shared rationale, and to answer questions about why specific letterforms look the way they do. It is deliberately candid about where the evidence is strong, where it rests on professional consensus, and where reasonable designers could still disagree.
...
> **Working principle** Design for a slightly out-of-focus image. Every decision below is justified by how well a letterform holds its identity when blur and crowding erode its finer detail — which is exactly the condition CorneaDev Mono's readers face when working at small sizes without optimal correction.
...
The zero deserves a specific, recorded decision. Under blur, an undifferentiated 0 and O become identical, and context does not always rescue the reader (passwords, hashes, identifiers). The literature supports a slash, an interior dot, or a distinctly different shape. A known trade-off: a dotted zero can be mistaken for an 8 on a low-resolution display — and blur effectively turns any display into a low-resolution one. CorneaDev Mono should favour a slashed zero as the default, optionally exposing a dotted variant through the standard OpenType 'zero' feature so users in scripts where the slash collides with Ø can opt out.
